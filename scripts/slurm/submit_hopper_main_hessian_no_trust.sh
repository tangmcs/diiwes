#!/bin/bash
#SBATCH --job-name=hop_main_hess
#SBATCH --output=job_outputs/hopper_main_hess_%A_%a.out
#SBATCH --error=job_outputs/hopper_main_hess_%A_%a.err
...
if [ -n "${PAPER_ITERATIONS:-}" ]; then
  echo "PAPER_ITERATIONS is forbidden: this protocol is fixed at 500 updates." >&2
  exit 2
fi

CONFIG_UPDATES=$(python -c \
  'import sys; from experiments.train import load_config; print(load_config(sys.argv[1]).get("n_iterations"))' \
  "$CONFIG")
if [ "$CONFIG_UPDATES" != "500" ]; then
  echo "$CONFIG must specify exactly 500 updates; found $CONFIG_UPDATES." >&2
  exit 2
fi
...
export MPLCONFIGDIR="/tmp/${USER:-user}_matplotlib_hopper_main_hess_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "$MPLCONFIGDIR"

# Locked 2 x 2 x 2 x 10 matrix. Seed is the fastest-changing index.
...
if [ "$TOTAL_TASKS" -ne 80 ]; then
  echo "Internal protocol error: expected exactly 80 tasks, found $TOTAL_TASKS." >&2
  exit 2
fi
...
echo "========================================"
echo "Hopper main Hessian, no-trust paired sweep"
echo "========================================"
