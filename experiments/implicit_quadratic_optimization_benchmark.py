...
def _amplification(step: np.ndarray, explicit: np.ndarray) -> float:
    ...
    if not np.isfinite(explicit_norm) or not np.isfinite(step_norm):
        return MAX_FINITE_DIAGNOSTIC
    if explicit_norm == 0.0:
        return 0.0 if step_norm == 0.0 else MAX_FINITE_DIAGNOSTIC
...
def _structured_steps(
    ...
    # Preserve the equal-norm control even after the optimizer has converged to
    # machine-scale updates.  Treating every norm below an absolute tolerance
    # as zero leaves a small but nonzero explicit step unscaled and violates the
    # control's defining invariant.  The stable norm calculation above remains
    # well behaved for subnormal finite values, so only an exactly zero step
    # needs a division guard.
    scale = 1.0 if explicit_norm == 0.0 else structured_norm / explicit_norm
...
def _diagonal_structured_steps(
    gradient: np.ndarray,
    diagonal_curvature: np.ndarray,
    alpha: float,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, float]:
    """Projected diagonal implicit step and its equal-norm scalar control."""
    concave = np.maximum(-np.asarray(diagonal_curvature, dtype=np.float64), 0.0)
    denominator = 1.0 + alpha * concave
    explicit = alpha * gradient
    structured = explicit / denominator
    explicit_norm = _stable_norm(explicit)
    structured_norm = _stable_norm(structured)
    ...
    return structured, isotropic, denominator, float(norm_error)
...
def propose_step(
    ...
    if method == "oracle_true_diagonal":
        return _diagonal_proposal(
            gradient, case.diagonal, alpha, singular_tolerance
        )
    ...
    if method in {
        ...
        structured, isotropic, denominator, norm_error = (
            _diagonal_structured_steps(gradient, estimates.diagonal, alpha)
        )
        step = (
            structured
            if method == "concave_projected_diagonal"
            else isotropic
        )
        margin = float(np.min(denominator))
        scale = _amplification(step, explicit)
        inverse_norm = (
            scale
            if method == "diagonal_norm_matched_isotropic"
            else float(1.0 / margin)
        )
        return Proposal(
            step=step,
            denominator_min_abs=(
            ...
            failure_reason=(
                "" if np.all(np.isfinite(step)) else "nonfinite_safe_step"
            ),
...
def _method_metadata() -> dict[str, dict[str, Any]]:
    return {
        ...
        "oracle_true_diagonal": {
            "curvature": "known_hessian_diagonal_with_sampled_gradient",
        ...
        "concave_projected_diagonal": {
            "curvature": "max(-sampled_or_exact_diagonal_statistic,0)",
            "update": "coordinatewise_denominator_at_least_one",
        },
        "diagonal_norm_matched_isotropic": {
            "curvature": "used_only_to_compute_projected_diagonal_step_norm",
            "update": "explicit_direction_scaled_to_projected_diagonal_step_norm",
        },
        "oracle_block_approximation_signed": {
